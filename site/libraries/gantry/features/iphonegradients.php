...
class GantryFeatureIphoneGradients extends GantryFeature
{
	...
	function init()
	{
		...
		foreach ($filtered as $filter) {
			...
			$css .= "#rt-" . $position . " .container, #rt-" . $position . " .container {background: -webkit-gradient(" . $type . ", " . $dirStart . ", " . $dirEnd . ", from(rgba(" . $this->_hex2rgb($from) . ", " . $opacity['from'] . ")), to(rgba(" . $this->_hex2rgb($to) . ", " . $opacity['to'] . "))) !important;}\n";
		}
